      ...
      <div className="h-5/6 w-full flex justify-start items-center flex-col p-5  ">
        {itemPrice == 0
          ...
          : cartItems.map((items: { id: number; name: string; price: number; qty: number }) => (
              <div
                key={items.id}
                className="w-2/6 h-[10%] rounded hover:rounded-sm bg-zinc-700 hover:bg-zinc-600 transition flex justify-center items-center  p-2 m-2"
              >
                <div className="w-1/3 h-full flex justify-center items-center flex-col">
                  ...
                  <div className="w-full h-1/2 font-light text-base">{items.price} $</div>
                </div>
                <div className="w-2/3 h-full flex justify-evenly items-center text-lg font-semibold">
                  <button
                    onClick={() => decreaseCount({ ...items })}
                    className="bg-neutral-800 hover:bg-neutral-700 transition w-1/6 h-3/4 rounded hover:rounded-sm"
                  >
                    -
                  </button>
